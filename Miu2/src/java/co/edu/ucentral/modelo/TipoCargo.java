...
/**
 * Clase que define la persistencia del objeto tipo cargo
 * @author David Acosta
 * @version 1.0
 */
@Entity
@Table(name = "tipo_cargo")
@NamedQueries({@NamedQuery(name = "TipoCargo.findAll", query = "SELECT t FROM TipoCargo t ORDER BY t.idTipocargo"), @NamedQuery(name = "TipoCargo.findByIdTipocargo", query = "SELECT t FROM TipoCargo t WHERE t.idTipocargo = :idTipocargo"), @NamedQuery(name = "TipoCargo.findByNomTipocargo", query = "SELECT t FROM TipoCargo t WHERE t.nomTipocargo = :nomTipocargo")})
